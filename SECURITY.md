...
- OIDC validated neither the ID token nor the nonce and derived redirect URIs from `r.TLS`. `[repaired in CP30 Campaign 3]` OIDC now uses `github.com/coreos/go-oidc` for standards-compliant discovery, keyset retrieval and ID-token verification (signature, issuer, audience, expiry), enforces one-time state consumption and nonce equality, requires a verified email (with userinfo fallback only when the ID token omits email), and keeps local password login as an independent recovery path. `[Campaign 3/4 closure]` Each authorization transaction is bound to the initiating browser with a short-lived HttpOnly SameSite=Lax cookie (follows the trusted-proxy Secure decision), and the browser-bound state is consumed atomically (`DELETE ... WHERE state AND browser`), so a different browser can neither establish a session nor destroy the legitimate browser's transaction. The OIDC callback origin is canonical and fail-closed: it comes only from `WEBFLEET_PUBLIC_URL` (required to enable/use OIDC), never from `Host`, `X-Forwarded-Host` or `Forwarded`. Real-provider interoperability remains an external CP30 release gate; a local standards-shaped provider simulation covers the adversarial cases.
- `webfleet backup`/`restore` always addressed the SQLite path regardless of the configured provider. `[repaired in CP30 Campaign 5]` Backup/restore is provider-aware: SQLite keeps its native path (validated integrity + schema on restore); PostgreSQL uses provider-native pg_dump/pg_restore with credentials passed only via `PGPASSWORD`, bounded subprocesses, clean tool-absence detection, temp-file security and destructive-change rehearsals for both providers. See `docs/hardening/database.json`.
- Split workers had no coordination, so two worker-capable processes could perform the same due work. `[repaired in CP30 Campaign 5]` The scheduler now uses a database claim/lease (`job_leases`, migration 26) with unique owner identities, expiry and reclamation, proven equivalent on SQLite and PostgreSQL by two-worker adversarial tests.
- Password hashing used a cgo `libargon2` binding that blocked macOS/Windows builds. `[repaired in CP30 Campaign 6]` Hashing is now pure-Go Argon2id (`golang.org/x/crypto/argon2`) using the same PHC format and parameters, so existing libargon2 hashes verify unchanged (migration fixtures in the password tests). The module graph is clean (`go mod verify` passes, clean-checkout build/test works without `-mod=mod`).
