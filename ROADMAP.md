...
**Campaign 8 (implemented, pending review):** the application frontend gained an accessibility pass (status live-region announcements, visible `:focus-visible` outlines, and focus management on route changes with the embedded frontend rebuilt); the public website docs were corrected to exact shipped truth (analytics Origin/abuse contract, hosting/OIDC, current-status wording); and a stranger-style rehearsal from a built release archive (`scripts/rehearse.sh`) exercises fresh SQLite setup, admin creation, sites/groups/tags, a real guarded monitoring check, Audit, analytics, API tokens, webhooks, backup, restart and persistence. The owner's ordinary-user dogfood and real external-provider OIDC interoperability remain outstanding CP31/CP30 gates.
