...
- CP30 Campaign 6 (release portability and reproducibility) is **complete and reviewed**. The module graph is repaired (`go mod verify` passes; clean-checkout build/test needs no `-mod=mod`); password hashing is pure-Go Argon2id with legacy libargon2 hashes verifying unchanged; Linux/macOS/Windows amd64+arm64 cross-compile; systemd install creates the service account and ownership idempotently and rolls back cleanly on partial failure, rehearsed by the CI systemd-lifecycle job; release building and read-only verification are separate (`scripts/release.sh`, exact-set `scripts/verify-release.sh`, read-only `scripts/verify-gh-release.sh`, GitHub build-provenance attestations) and never publish. CP30 Campaign 7 (analytics hardening, deployment idempotency, tag filtering, scale) is **implemented, pending review**. Analytics enforces the browser-tracker Origin contract (server-side mode opt-in), uses trusted-proxy client identity, applies bounded per-address/property rate limiting (no raw-IP persistence) and validates event kind/payload shape. Deployment deduplication uses a partial unique index so empty `external_id` events never collapse and PG sequences are synced after the rebuild. Tag filtering is wired and tested end-to-end. Scale measurements at 100/1k/10k sites are recorded in `SCALE.md`; the no-new-infrastructure decision is unchanged. CP30 Campaign 8 (UX/a11y, docs, pre-release dogfood) is **implemented, pending review**. A deterministic chromedp browser suite proves dialog initial focus + Escape/focus-return, announced form errors, mobile-nav aria-expanded and keyboard tabbing; the frontend gained live-region status, visible focus and dialog focus-to-primary-field. The stranger rehearsal (`scripts/rehearse.sh`) now covers SQLite and PostgreSQL setup, incident open/ack/recover, webhook outbox wiring, Audit, analytics, tokens, backup/destructive-change/restore, verified update/rollback and restart persistence. The final Campaign 5/6 closure corrections (provider-neutral claim matrix on SQLite+PG, env-`WEBFLEET_DATABASE_URL` startup test, exact-set/attestation post-release verifier with mocked-gh contract tests, deterministic systemd failure injection) are in place; the systemd-lifecycle and GitHub attestation jobs still need a real GitHub run to turn green. The owner's ordinary-user dogfood and real external-OIDC interoperability remain outstanding gates; CP31 stays blocked.
